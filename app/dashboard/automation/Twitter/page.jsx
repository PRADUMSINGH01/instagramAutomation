...
const TweetEditorModal = ({ isOpen, onClose, onSave, tweet }) => {
    ...
    const handleSave = () => { onSave({ id: tweet?.id || Date.now(), text, imageUrl: imagePreview, date: new Date(scheduleDate) }); onClose(); };
    ...
    return (
        <div className={`fixed inset-0 z-40 transition-opacity duration-300 ${isOpen ? 'bg-black/40' : 'bg-transparent pointer-events-none'}`} onClick={onClose}>
            <div className={`fixed top-0 right-0 h-full w-full max-w-lg bg-zinc-50 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between p-4 border-b border-dotted border-black bg-white"><h3 className="text-xl font-bold">{tweet ? 'Edit Tweet' : 'Schedule a Tweet'}</h3><button onClick={onClose} className="p-2 rounded-full hover:bg-zinc-100"><XIcon className="w-5 h-5" /></button></div>
                ...
                <div className="p-4 bg-white border-t border-dotted border-black"><button onClick={handleSave} className="w-full px-4 py-3 bg-black text-white rounded-lg hover:bg-zinc-800 font-bold">Save Tweet</button></div>
            </div>
        </div>
    );
};
...
const TweetScheduler = () => {
    const [tweets, setTweets] = useState([
        { id: 1, text: 'Excited to announce our Q3 results! A big thank you to the entire team for their hard work. #business #results', imageUrl: 'https://placehold.co/1200x628/000000/FFFFFF?text=Q3+Results', date: new Date(Date.now() + 86400000 * 3) },
    ...
    const handleEdit = (tweet) => { setEditingTweet(tweet); setIsEditorOpen(true); };
    const handleNew = () => { setEditingTweet(null); setIsEditorOpen(true); };
    const handleDelete = (id) => setTweets(tweets.filter(p => p.id !== id));

    return (
        <div>
            <div className="flex justify-between items-center mb-6"><h3 className="font-bold text-xl">Scheduled Tweets</h3><button onClick={handleNew} className="px-4 py-2 bg-black text-white rounded-lg flex items-center gap-2 hover:bg-zinc-800"><Plus className="w-4 h-4" /> Schedule Tweet</button></div>
            <Card>
                <div className="space-y-4">
                    {tweets.length > 0 ? tweets.sort((a,b) => a.date - b.date).map(tweet => (
                        <div key={tweet.id} className="flex items-start gap-4 p-3 border border-dotted border-zinc-300 rounded-lg">
                            {tweet.imageUrl && <img src={tweet.imageUrl} alt="Tweet image" className="w-28 h-28 object-cover rounded-md flex-shrink-0" />}
                            <div className="flex-grow">
                                <p className="text-sm text-zinc-800 line-clamp-3">{tweet.text}</p>
                                <div className="mt-2 text-sm text-zinc-500 flex items-center gap-2"><Clock className="w-4 h-4" /><span>{tweet.date.toLocaleString()}</span></div>
                            </div>
                            <div className="flex flex-col gap-2"><button onClick={() => handleEdit(tweet)} className="p-2 text-zinc-500 hover:bg-zinc-100 rounded-full"><Pencil className="w-4 h-4" /></button><button onClick={() => handleDelete(tweet.id)} className="p-2 text-zinc-500 hover:bg-zinc-100 rounded-full"><Trash2 className="w-4 h-4" /></button></div>
                        </div>
                    )) : <p className="text-center text-zinc-500 py-8">No tweets scheduled.</p>}
                </div>
            </Card>
            <TweetEditorModal isOpen={isEditorOpen} onClose={() => setIsEditorOpen(false)} onSave={handleSaveTweet} tweet={editingTweet} />
        </div>
    );
};
    ...
    const handleAIGeneration = async () => {
        ...
        try {
            ...
            if (result.candidates?.[0]?.content?.parts?.[0]?.text) {
                ...
                setGeneratedTweet("Sorry, I couldn't generate a tweet for that topic. Please try again.");
            }
        } catch (error) {
            console.error("Error with Gemini API:", error);
            setGeneratedTweet("An error occurred while generating the tweet. Please check the console.");
        } finally {
            ...
            <Card>
                <div className="space-y-4">
                    <div>
                        ...
                        <p className="text-sm text-zinc-600 mb-4">Enter a topic, idea, or keywords, and let AI craft a professional tweet for you.</p>
                        <textarea
                            id="ai-prompt"
                            value={prompt}
...
const ReplyRuleEditorPanel = ({ isOpen, rule, onSave, onClose }) => {
    ...
    const toggleTweetSelection = (tweetId) => setSelectedTweetIds(prev => prev.includes(tweetId) ? prev.filter(id => id !== tweetId) : [...prev, tweetId]);
    ...
    const mockTweets = Array.from({ length: 8 }, (_, i) => ({ id: 301 + i, imageUrl: `https://placehold.co/400x200/${Math.floor(Math.random()*16777215).toString(16)}/FFFFFF?text=Tweet+${i+1}` }));
        ...
        <div className={`fixed inset-0 z-40 transition-opacity duration-300 ${isOpen ? 'bg-black/40' : 'bg-transparent pointer-events-none'}`} onClick={onClose}>
            <div className={`fixed top-0 right-0 h-full w-full max-w-lg bg-zinc-50 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-grow p-6 space-y-6 overflow-y-auto">
                    <div className="space-y-2"><h3 className="text-lg font-bold">1. Select Tweets</h3><div className="flex gap-2"><button onClick={() => setTweetSelection('all')} className={`flex-1 p-3 rounded-lg border border-dotted ${tweetSelection === 'all' ? 'bg-zinc-100 border-black' : 'border-zinc-300'}`}>All Tweets</button><button onClick={() => setTweetSelection('specific')} className={`flex-1 p-3 rounded-lg border border-dotted ${tweetSelection === 'specific' ? 'bg-zinc-100 border-black' : 'border-zinc-300'}`}>Specific Tweets</button></div>{tweetSelection === 'specific' && (<div className="grid grid-cols-4 gap-3 max-h-52 overflow-y-auto p-2 bg-white rounded-lg border border-dotted">{mockTweets.map(tweet => (<div key={tweet.id} onClick={() => toggleTweetSelection(tweet.id)} className="relative cursor-pointer aspect-video group"><img src={tweet.imageUrl} className="w-full h-full object-cover rounded-md" /><div className={`absolute inset-0 bg-black transition-opacity rounded-md ${selectedTweetIds.includes(tweet.id) ? 'opacity-40' : 'opacity-0 group-hover:opacity-20'}`}></div><div className={`absolute top-1 right-1 w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedTweetIds.includes(tweet.id) ? 'bg-black border-black text-white' : 'bg-white/50 border-white'}`}><Check className="w-3 h-3" /></div></div>))}</div>)}</div>
                    <div className="space-y-2"><h3 className="text-lg font-bold">2. Set Trigger</h3><div className="flex gap-4"><div onClick={() => setTriggerType('all_replies')} className="flex-1 p-4 border border-dotted rounded-lg cursor-pointer flex items-start gap-3 bg-white">{triggerType === 'all_replies' ? <CheckCircle2 className="w-6 h-6 text-black mt-0.5" /> : <Circle className="w-6 h-6 text-zinc-400 mt-0.5" />}<div><h4 className="font-bold">All Replies</h4><p className="text-sm text-zinc-600">Reply to every comment.</p></div></div><div onClick={() => setTriggerType('specific_keywords')} className="flex-1 p-4 border border-dotted rounded-lg cursor-pointer flex items-start gap-3 bg-white">{triggerType === 'specific_keywords' ? <CheckCircle2 className="w-6 h-6 text-black mt-0.5" /> : <Circle className="w-6 h-6 text-zinc-400 mt-0.5" />}<div><h4 className="font-bold">Keywords</h4><p className="text-sm text-zinc-600">Only reply to specific words.</p></div></div></div>{triggerType === 'specific_keywords' && (<div className="pt-2"><input type="text" value={triggerKeywords} onChange={e => setTriggerKeywords(e.target.value)} className="w-full p-2 bg-white border border-dotted rounded-lg" placeholder="e.g. congrats, hiring, interested" /></div>)}</div>
...
const ReplyAutomator = ({ rules, setRules }) => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6"><h3 className="font-bold text-xl">Reply Automation Rules</h3><button onClick={handleNewRule} className="px-4 py-2 bg-black text-white rounded-lg flex items-center gap-2 hover:bg-zinc-800"><Plus className="w-4 h-4" /> New Rule</button></div>
            <div className="space-y-4">
                {rules.length > 0 ? rules.map(rule => (
                    <Card key={rule.id} className="!p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="flex-grow"><h4 className="font-bold">{rule.name}</h4><div className="flex items-center gap-4 text-sm text-zinc-600 mt-1 flex-wrap"><span className="flex items-center gap-1.5"><List className="w-4 h-4" /> {rule.tweetSelection === 'specific' ? `${rule.selectedTweetIds.length} Tweets` : 'All Tweets'}</span><span className="flex items-center gap-1.5"><Tag className="w-4 h-4" /> {rule.triggerType === 'specific_keywords' ? `${rule.triggerKeywords.length} Keywords` : 'All Replies'}</span></div></div>
                ...
                )) : <Card className="text-center py-12 border-2 border-dashed"><p className="text-zinc-500">No reply automation rules created yet.</p></Card>}
            </div>
            <ReplyRuleEditorPanel isOpen={isPanelOpen} rule={editingRule} onSave={handleSaveRule} onClose={() => setIsPanelOpen(false)} />
        </div>
    );
};
    ...
    if (!session) {
        return (
            <div className="flex flex-col h-full">
                <SectionHeader icon={TwitterIcon} title="Twitter Automation" />
                <Card className="flex-grow flex flex-col items-center justify-center text-center border-2 border-dashed bg-zinc-50">
                    <TwitterIcon className="w-16 h-16 text-zinc-400 mb-4" />
                    <h3 className="text-2xl font-bold">Connect your Twitter Account</h3>
                    <p className="text-zinc-600 mt-2 max-w-sm">To get started with scheduling tweets and automating replies, you first need to connect your Twitter account.</p>
                    <button onClick={() => signIn('twitter')} className="mt-6 px-6 py-3 bg-black text-white rounded-lg flex items-center gap-2 hover:bg-zinc-800 transition-colors">
        ...
        <div className="flex flex-col h-full">
            <SectionHeader icon={TwitterIcon} title="Twitter Automation">
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => signOut()} className="px-3 py-1 text-sm border border-black rounded-md hover:bg-zinc-100">
                        Disconnect
                    </button>
                </div>
            </SectionHeader>
            <div className="border-b border-dotted border-black mb-6">
                <nav className="flex gap-4 overflow-x-auto">
                    {['Tweet Scheduler', 'AI Assistant', 'Reply Automation'].map(tab => (
            ...
            <div>
                ...
                {activeTab === 'Reply Automation' && <ReplyAutomator rules={rules} setRules={setRules} />}
            </div>
        </div>
    );
};
